...
#from speeqeweb.caching.caching import cache_update, cache_evict

class Venue(models.Model):
    name = models.CharField(max_length=50)
    slug = models.SlugField()
    
    # TODO: only select users with user_level 8
    merchant = models.ForeignKey(User) # , limit_choices_to={'id': UserProfile.filter(user_level=8)})
    ...
    def get_absolute_url(self):
        return ('catalog_venue', (), { 'venue_id': self.id, 'venue_slug': self.slug })
    
    def get_absolute_url_admin(self):
        return '/admin/catalog/venue/%d/' % (self.id)
...
class Category(models.Model):
    ...
    def venue_permalink(self):
        venue = Venue.objects.get(id=self.venue_id)
        return '<a href="%s">%s</a>' % (venue.get_absolute_url_admin(), venue.name)
    venue_permalink.allow_tags = True
    venue_permalink.admin_order_field = 'venue'
    venue_permalink.short_description = 'Venue'
...
class Product(models.Model):
    ...
    @models.permalink
    def get_absolute_url(self):
        return ('just_catalog_product', (), { 'product_slug': self.slug })
