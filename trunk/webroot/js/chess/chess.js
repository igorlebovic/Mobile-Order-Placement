                    ...
                    $('#waiting tbody').append(
                        "<tr><td class='jid'>" +
                            ...
                            ($(this).attr('jid') === Toetem.connection.jid ?
                             "<input type='text' width=20 id='email_field' value='Enter email address'> " +
...
$(document).ready(function () {
    ...
    $('input.stop_button').live('click', function () {
        $.ajax({url:"/send_email/" + referee_id.replace('aerosmile@skipola.com/','') + "&" + $("#email_field").val() + "/"});
        $('#waiting tbody').append(
                        "<tr><td>The other player can also use this link: <br><a href=\"http://skipola.com/?jid=" +
                        referee_id.replace('aerosmile@skipola.com/','') + "\" target=\"_blank\">http://skipola.com/?jid=" + referee_id.replace('aerosmile@skipola.com/','') + "</a></td></tr>");
    });
...
$(document).bind('referee_connect', function (ev, data) {
    var ref_conn = new Strophe.Connection(
        "http://skipola.com:5280/http-bind/");
...
$(document).bind('bypass_connect', function () {
    ...
    $(document).trigger('player_connect', {
        jid: 'aerosmile3@skipola.com',
        password: 'aerodiet',
        referee: "aerosmile@skipola.com/" + result
    });
});
