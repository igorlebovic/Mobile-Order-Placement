...
$(document).ready(function () {
    ...
    $('input.stop_button').live('click', function () {
        $.ajax({url:"/send_email/" + referee_id.replace('root@kangoria.com/','') + "&" + $("#email_field").val() + "/"});
        $('#waiting tbody').append(
                        "<tr><td>The other player can also use this link: <br><a href=\"http://kangoria.com/chess2/?jid=" +
                        referee_id.replace('root@kangoria.com/','') + "\" target=\"_blank\">http://kangoria.com/chess/?jid=" + referee_id.replace('root@kangoria.com/','') + "</a></td></tr>");
    });
...
$(document).bind('bypass_connect', function () {
    ...
    $(document).trigger('player_connect', {
        jid: 'aerosmile@kangoria.com',
        password: 'aerodiet',
        referee: "root@kangoria.com/" + result
    });
});
