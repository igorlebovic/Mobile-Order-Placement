...
// skipola code
    $('#accept_order').live('touchstart', function () {
        var customer = $(this).attr('name');
        app.on_game_start(customer);
    });
    $('#finalize_order').live('touchstart', function () {
        var customer = $(this).attr('name');
        app.on_game_start(customer);
    });
    $('#decline_order').live('touchstart', function () {
        var customer = $(this).attr('name');
        app.on_game_start(customer);
    });
    $('#cancel_order').live('click', function () {
        var customer = $(this).attr('name');
        app.cancel_order(customer);
    });
		...
		*/
		...
		$('#layer1_form').ajaxForm({
				...
				success: function(responseXML) 
				{
					...
					{
					    $("#form_message").empty();
					    $("#form_message").append("Successfully logged in.");
					    $("#layer1").hide();
					    window.location.reload();
					}
